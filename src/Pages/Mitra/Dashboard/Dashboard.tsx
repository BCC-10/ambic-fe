...
const Content: React.FC<Content> = ({ color, header, number, icon }) => {
  return (
    <div
      className={`${color} w-[30%] h-[20%] p-3 flex items-center justify-between rounded-xl max-xl:w-[40%] max-lg:w-[50%] max-md:w-[70%] max-sm:w-full`}
    >
    ...
    const fetchStatistic = async () => {
      ...
      try {
        const response = await axios.get(
          import.meta.env.VITE_API_URL + '/api/v1/partners/statistics',
          {
            headers: {
          ...
          <div className="w-[80%] h-[85%] flex items-center justify-center gap-5 flex-col">
            ...
            <div className="w-full h-full flex flex-wrap items-center justify-center gap-5 max-xl:flex-col">
              <Content
                color="bg-[#FFB4B4]"
                header="Umpan balik"
                number={statistic.total_ratings}
                icon={<LuMessageSquareWarning size={50} />}
              />
              <Content
                color="bg-[#FDFFA0]"
                header="Total Produk"
              ...
              <Content
                ...
                number={statistic.total_transactions}
                icon={<IoCartOutline size={50} />}
              />
              <Content
                color="bg-[#96FFA7]"
                header="Total Pendapatan"
                number={formatRupiah(statistic.total_revenue)}
