...
const TableTransaction = ({ className }: { className?: string }) => {
    ...
    const fetchTransactions = async () => {
      try {
        const response = await axios.get(
          import.meta.env.VITE_API_URL + '/api/v1/partners/transactions',
          {
            headers: {
        ...
        const maxId =
          response.data.length > 0
            ? Math.max(...response.data.payload.transactions.map((p) => p.id))
            : 0;
    ...
    try {
      const response = await axios.patch(
        import.meta.env.VITE_API_URL +
          `/api/v1/transactions/${selectedTransaction?.id}`,
        {
          status: 'finish',
        },
        {
  ...
  const actionBodyTemplate = (rowData: Transaction) => (
    <div className="flex gap-2">
      ...
      />
      <Button
        onClick={() => openInfoDialog(rowData)}
        icon="pi pi-eye"
        severity="info"
      />
    </div>
  ...
  return (
    <div className={className}>
          ...
          <div className="p-4 bg-gray-100 rounded-lg shadow-md">
            <div className="flex flex-col gap-3 ">
              ...
              <DetailItem
                ...
                value={selectedTransaction.payment.payment_type}
              />
              <DetailItem
                label="Status"
                value={
                  selectedTransaction.status === 'waiting for payment'
                    ? 'Menunggu Pembayaran'
                    : selectedTransaction.status === 'process'
                    ? 'Sedang Proses'
                    : selectedTransaction.status === 'finish'
                    ? 'Selesai'
                    : ''
                }
              />
              <DetailItem
                label="Total"
                value={`Rp ${selectedTransaction.total.toLocaleString()}`}
              />
              <DetailItem
                label="Waktu"
                value={new Date(
              ...
              />
              <button
                className="w-[30%] items-center self-center bg-teal-700/85 text-white rounded-full py-2 focus:outline-none cursor-pointer transition duration-300 hover:bg-teal-500 ease-in"
      ...
      >
        {selectedTransaction && (
          <DataTable
            value={selectedTransaction.items}
            tableStyle={{ minWidth: '100%' }}
            scrollable
            scrollHeight="flex"
          >
            ...
            <Column
              field="final_price"
              header="Harga"
              body={(data) => `Rp ${data.final_price.toLocaleString()}`}
            />
            ...
            <Column
              ...
              header="Subtotal"
              body={(data) =>
                `Rp ${(data.final_price * data.stock).toLocaleString()}`
              }
            />
          </DataTable>
        )}
