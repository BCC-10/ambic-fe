...
const Daftar: React.FC = () => {
    ...
    const fetchLocations = async (searchTerm: string) => {
        ...
        try {
            const response = await axios.get(
                'https://ambic.live:443/api/v1/locations',
                {
                    params: {
                        query: searchTerm,
                        lat: latitude,
    ...
    useEffect(() => {
        const fetchBusinesType = async () => {
            ...
            try {
                const response = await axios.get(
                    'https://ambic.live:443/api/v1/business-types',
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
                        },
        ...
        <main className="min-h-screen flex flex-col overflow-hidden w-full ">
            ...
            <div className="flex flex-row items-start justify-start">
                <div className="w-[25%] min-h-[120vh] flex flex-col items-center gap-4 bg-teal-700/85 pt-60 max-md:justify-start max-md:pt-120 max-lg:pt-0 max-lg:justify-center ">
                    <h1 className="font-Poppins text-3xl text-white font-semibold max-lg:text-center">
                        Mitra Ambic
                    </h1>
                    <div className="flex items-center justify-center flex-col gap-20 ">
                        {cards.slice(0, 3).map((_, idx) => (
                            <img
                                ...
                                src={_.description}
                                alt=""
                                className="w-1/2 bg-none drop-shadow-xl"
                            />
                        ))}
                    </div>
                </div>
                <div className="bg-[#FFF8F4] w-full min-h-[116vh] mt-10 flex flex-col items-center justify-center gap-7 relative max-md:min-h-[116vh] max-md:justify-start max-md:pt-80">
                    <div className="w-[50%] h-auto flex items-center justify-center ">
                        <h1 className=" w-full text-center font-Poppins text-4xl font-semibold">
                            Form Pendaftaran Mitra Ambic
                        </h1>
                    </div>
                    <div className="w-full flex items-center justify-center gap-6 flex-col h-auto bg-none drop-shadow-xl">
                        <div className="w-3/4 h-auto flex items-center justify-start flex-col gap-5">
                            ...
                            <div className="flex items-start flex-col w-full justify-center gap-2">
                                <div className="flex gap-2 justify-start items-start ">
                                    <label
                                        ...
                                        className="font-Poppins text-md font-semibold text-teal-700 ml-72 max-2xl:ml-49 max-xl:ml-10 max-lg:ml-6 max-sm:ml-0"
                                    >
                                        Jenis Bisnis
                                    </label>
                                </div>
                                <div className="flex w-full items-center justify-center">
                                    <select
                                        ...
                                        value={
                                            dataUsers.business_type_id as string
                                        }
                                        onChange={handleInputChange}
                                        disabled={loading}
                                    >
                            ...
                            <div className="w-full flex items-center justify-center ">
                                <div
                                    {...getRootProps()}
                                    className="relative w-[55%] rounded-xl flex flex-col  cursor-pointer gap-3  max-sm:w-[79%] max-xl:w-[90%] "
                                >
                                    <label
                                        className={`font-semibold font-Poppins text-teal-700/85`}
                                    >
                                        Masukkan Gambar Bisnis
                                    </label>
                                    <div
                                        className={`w-full flex items-center pl-17 py-5 max-sm:pl-14 border-2 border-dashed border-gray-400 hover:bg-gray-100 rounded-xl ${
