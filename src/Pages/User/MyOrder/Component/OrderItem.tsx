...
const OrderItem: React.FC<OrderItem> = ({
  ...
  return (
    <main
      onClick={onClick}
      className="flex items-start flex-col p-3 cursor-pointer"
    >
      <div className="relative min-h-[370px] max-w-[300px] flex items-start justify-start flex-col bg-white drop-shadow-xl rounded-3xl">
        <div className="relative w-full flex items-center justify-center rounded-xl h-1/4  ">
          ...
          <p className="bg-white/80 px-1.5 absolute rounded-full right-4 bottom-3">
            {(product?.distance ? Number(product.distance) / 1000 : 0).toFixed(
              2
            )}
            Km
          </p>
        ...
        <div className="w-full h-1/4 p-5 flex flex-col">
          <label htmlFor="" className="font-Poppins font-normal text-teal-700">
        ...
        <div className="relative w-full h-full p-5 flex flex-col gap-3">
          <h3 className="text-base font-Poppins font-medium">
            Rp{new Intl.NumberFormat('id-ID').format(product?.final_price ?? 0)}{' '}
          ...
          <h5 className="absolute top-15 left-13 text-gray-500 text-sm ">
            {product?.star} {product?.count_rating}
          </h5>
          <div className="w-26 h-7 bg-teal-700 rounded-full flex items-center justify-center drop-shadow-lg transition-transform duration-330 hover:scale-95 cursor-pointer absolute right-5 bottom-4 p">
            <button
              className="text-white font-Poppins text-sm font-medium cursor-pointer"
              onClick={() => {
                setVisible(true);
              }}
            >
      ...
      >
        <div className="flex items-center justify-center flex-col gap-4">
          ...
          />
          <Rating
            value={createRating.star}
            onChange={(e) =>
              setCreateRating((prev) => ({ ...prev, star: e.value ?? 0 }))
            }
            stars={5}
            cancel={false}
          />
        ...
        <div style={{ margin: '20px 0' }}>
          <InputText
            ...
            onChange={(e) =>
              setCreateRating((prev) => ({ ...prev, feedback: e.target.value }))
            }
            style={{ width: '100%' }}
          />
        </div>
