...
const Cart: React.FC<Carts> = ({ isVisible }) => {
  ...
  // Kolom Total Harga
  const totalTemplate = (rowData: cartItem) =>
    `Rp ${new Intl.NumberFormat('id-ID').format(
      rowData.final_price * rowData.quantity
    )}`;
  ...
  const Total = () => {
    return (
      <h3 className="font-Poppins text-teal-700/85 font-bold text-lg w-full ">
        {' '}
        Rp {new Intl.NumberFormat('id-ID').format(checkedTotalPrice)}
      </h3>
    );
  };
      ...
      try {
        ...
        if (err.response.data.status_code === 400) {
          if (err.response.data.message.includes(`Insufficient stock for`)) {
            ...
            err.response.data.message.includes('Transaction items are missing')
          ) {
            Swal.fire({
            ...
            err.response.data.message.includes('The product with id')
          ) {
            const removedProductId =
              err.response.data.message.match(/\d+/)?.[0]; // Ambil ID produk dari pesan error
            if (removedProductId) {
              // Ambil cart dari localStorage
              const cart = JSON.parse(localStorage.getItem('cart') || '[]');
              // Filter produk yang masih tersedia
              const updatedCart = cart.filter(
                (item: cartItem) => item.id !== removedProductId
              );
      ...
      <div className="flex flex-col justify-center gap-7 items-center w-full ">
        ...
        >
          <Column
            ...
            style={{ width: '3rem' }}
            headerStyle={{ backgroundColor: 'var(--teal-700)', color: 'white' }}
          />
          <Column
            header="Produk"
          ...
          <Column
            ...
            header="Harga"
            body={(rowData) =>
              `Rp ${new Intl.NumberFormat('id-ID').format(rowData.final_price)}`
            }
            headerStyle={{ backgroundColor: 'var(--teal-700)', color: 'white' }}
          />
          <Column
            ...
            bodyStyle={{ position: 'relative', left: '50px' }}
          />
          <Column
            header="Total"
            body={totalTemplate}
        ...
        {isVisible === true ? (
          <button
            className=" top-50 bg-teal-700/85 right-150 px-4 py-2 text-white font-Poppins font-semibold text-lg rounded-full drop-shadow-lg transition-transform duration-200 hover:scale-95 cursor-pointer w-50"
