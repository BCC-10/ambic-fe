...
const OrderDescription = () => {
    ...
    useEffect(() => {
        const fatchingDescription = async () => {
            try {
                const response = await axios.get(`https://ambic.live:443/api/v1/products/${id}`, {
                    headers: {
            ...
            <div className=' min-h-[120vh] w-full flex flex-col items-center justify-center bg-[#FFF8F4] gap-7'>
                <div className='w-[50%] h-auto flex flex-row justify-start items-center gap-6'>
                    ...
                    <div className='w-full flex flex-col justify-start items-start gap-4'>
                        <h1 className='font-Poppins text-2xl font-semibold '>{product?.name}</h1>
                        <p className='w-full text-wrap text-base '>{product?.description}</p>
                        <div className='w-full h-auto flex items-center justify-center flex-col '>
                            <h2 className='flex gap-4'>
                                <span className='line-through text-gray-500 text-lg font-Poppins font-medium'>Rp{new Intl.NumberFormat("id-ID").format(product?.initial_price ?? 0)}</span>
                                <span className='text-teal-700 font-Poppins text-xl font-semibold'>Rp{new Intl.NumberFormat("id-ID").format(product?.final_price ?? 0)}</span>
                            </h2>
                        </div>
                        <div className='w-full h-auto flex items-center justify-center gap-3 '>
                            <img src={Stars} alt="" className='w-6'/>
                            <label htmlFor="" className='text-gray-500 font-Poppins text-xs relative top-1'>{product?.star ?? 0} ({product?.count_rating ?? 0})</label>
                        </div>
                        <div className='w-full h-auto flex items-center justify-center '>
                            <h5 className='font-medium '>Waktu Pengambilan, {formatTime(product?.pickup_time)} - {formatTime(product?.end_pickup_time)}</h5>
                        </div>
