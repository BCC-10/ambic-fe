  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await axios.get(
        import.meta.env.VITE_API_URL + '/api/v1/partners/products',
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
      ...
      const maxId =
        response.data.length > 0
          ? Math.max(...response.data.payload.products.map((p) => p.id))
          : 0;
      Swal.close();
  ...
  const editProduct = async (updateProduct: product) => {
    try {
      ...
      const response = await axios.patch(
        import.meta.env.VITE_API_URL + `/api/v1/products/${updateProduct.id}`,
        updateProduct,
        {
          headers: {
            'Content-Type': 'multipart/form-data',
